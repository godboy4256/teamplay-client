...
function ProjectManageUI(props: IProsManage) {
  const complete = props?.data?.fetchProject.tasks.filter((el: any) => {
    return el.is_complete;
  });
  const statusState =
    complete &&
    isNaN((complete.length / props?.data?.fetchProject.tasks.length) * 100)
      ? 0
      : complete &&
        (complete.length / props?.data?.fetchProject.tasks.length) * 100;
        ...
        <Wrapper paddingTop="0">
          <S.ProjectResponsiveWeb>
            ...
            <div>
              ...
              <S.ProjectManaBoard>
                {props.data?.fetchProject.boards.length === 0 ? (
                  <S.NonePost>게시물이 없습니다.</S.NonePost>
                ) : (
                  props.data?.fetchProject.boards.map((el: any) => {
                    return (
                      <ProjectTeamBoard
                        key={uuidv4()}
                        title={el.title}
